      ...
      <section className="bg-gradient-to-r from-primary to-primary/80 text-primary-foreground py-16 lg:py-24">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl opacity-90 max-w-3xl mx-auto">
            From emergency repairs to regular maintenance, our network of licensed professionals 
            is ready to help with any home service need.
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {serviceCategories.map((service, index) => (
              <ServiceCard
                ...
                onClick={() => window.location.href = `/services/${service.title.toLowerCase().replace(/\s+/g, '-').replace('&', 'and')}`}
                className="h-full"
              />
            ))}
          </div>
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            <div className="space-y-4">
              ...
              <p className="text-muted-foreground">All professionals are thoroughly vetted, licensed, and insured for your peace of mind.</p>
            </div>
            <div className="space-y-4">
              <Home className="w-12 h-12 text-primary mx-auto" />
              <h3 className="font-heading font-semibold">No Account Required</h3>
              <p className="text-muted-foreground">Skip the hassle of creating accounts. Just submit your request and we'll handle the rest.</p>
            </div>
            <div className="space-y-4">
              <Wrench className="w-12 h-12 text-primary mx-auto" />
              <h3 className="font-heading font-semibold">Quality Guaranteed</h3>
              <p className="text-muted-foreground">We stand behind our work with quality guarantees and customer satisfaction promises.</p>
            </div>
          </div>
        </div>
      </section>
