...
export function EmployeeDetailModal({
  ...
  const activeJobs = jobs.filter(job => job.status === "in-progress" || (job.status === "pending" && job.employeeAccepted));
  const pendingJobs = jobs.filter(job => job.status === "pending" && !job.employeeAccepted);
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Badge variant={application.experienceLevel === "Expert" ? "success" : 
                                   application.experienceLevel === "Intermediate" ? "info" : "warning"}>
                      ...
                      <Card key={job.id || job._id} className="hover:shadow-md transition-shadow">
                        <CardContent className="p-4">
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              <h4 className="font-semibold text-sm">{job.service || job.serviceType}</h4>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              <p className="flex items-center space-x-1">
                                <DollarSign className="h-3 w-3" />
                                <span>{job.estimatedCost ? `${job.estimatedCost} AED` : 'Not estimated'}</span>
                              </p>
                            </div>
                          </div>
                          ...
                          {job.employeeRemarks && (
                            <div className="mt-3 pt-3 border-t">
                              <label className="text-sm text-muted-foreground">Employee Remarks</label>
                              <p className="text-sm mt-1">{job.employeeRemarks}</p>
                            </div>
                          )}
